...
RSpec.describe WorkOrderForm, type: :model do
  ...
  it 'validates presence of aliquot attributes if work order is started' do
    ...
    expect(form.save(ActionController::Parameters.new(aliquot_attributes: attributes.except(:fragment_size)))).to be_falsey
    expect(form.errors).to_not be_empty

    expect(form.save(ActionController::Parameters.new(aliquot_attributes: attributes.except(:qc_state)))).to be_falsey
    expect(form.errors).to_not be_empty
  end
  ...
  it 'validates presence of library attributes if work order has been qced' do
    ...
    expect(form.save(ActionController::Parameters.new(library_attributes: attributes.except(:kit_number)))).to be_falsey
    expect(form.errors).to_not be_empty
  end
   ...
   it 'updates library and changes state if work order is qc and attributes present' do
    ...
    work_order.qc!
    form = WorkOrderForm.new(work_order)
    expect(form.save(ActionController::Parameters.new(library_attributes: attributes))).to be_truthy

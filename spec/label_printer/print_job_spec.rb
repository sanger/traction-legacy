...
RSpec.describe LabelPrinter::PrintJob, type: :model do
  ...
  context 'Print my barcode is down' do
    it 'label template should be nil if print my barcode is down' do
      allow(LabelPrinter::PrintMyBarcodeApi::LabelTemplate)
        .to receive(:where)
        .and_raise(JsonApiClient::Errors::ConnectionError.new(Rails.env))
      print_job = LabelPrinter::PrintJob.new(printer_name: 'name', aliquots: aliquots)
      expect(print_job.label_template_id).to eq nil
      expect(print_job.valid?).to be false
    end

    it 'execute should return false if print my barcode is down' do
      allow_any_instance_of(LabelPrinter::PrintJob).to receive(:find_label_template_id) { '1' }
      allow(LabelPrinter::PrintMyBarcodeApi::PrintJob)
        .to receive(:create)
        .and_raise(JsonApiClient::Errors::ConnectionError.new(Rails.env))
      print_job = LabelPrinter::PrintJob.new(printer_name: 'name', aliquots: aliquots)

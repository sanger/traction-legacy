...
RSpec.describe LabelPrinter::PrintJob, type: :model do
  ...
  let(:attributes)      { { printer_name: printer.name, label_template_id: 1, labels: LabelPrinter::Labels.new(WorkOrder.find(work_order_ids)).to_h } }
  ...
  it 'if the printing is successful returns an appropriate message' do
    ...
    label_printer = LabelPrinter::PrintJob.new(printer_name: printer.name, label_template_id: 1, work_orders: work_order_ids)
